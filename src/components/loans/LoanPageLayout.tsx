...
interface LoanPageLayoutProps {
  ...
  features: Feature[];
  featuresSectionTitleKey: string;
  featuresSectionDescKey: string;
  advantages: Advantage[];
  eligibilityKeys: string[];
  documentKeys: string[];
  ctaTitleKey: string;
  ctaDescKey: string;
  showCheckEligibility?: boolean;
...
const LoanPageLayout = ({
  ...
  descriptionKey,
  features,
  featuresSectionTitleKey,
  featuresSectionDescKey,
  advantages,
  eligibilityKeys,
  documentKeys,
  ctaTitleKey,
  ctaDescKey,
  showCheckEligibility = false,
  children,
}: LoanPageLayoutProps) => {
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className={`max-w-3xl ${isRTL ? 'mr-0 text-right' : ''}`}>
            ...
            <h1 className="text-display-sm md:text-display text-[hsl(var(--background))] mb-6">
              {t(titleKey)}{" "}
              <span className="text-accent">{t(highlightKey)}</span>
            </h1>
        ...
        <div className="container mx-auto px-4">
          <div className={`text-center max-w-3xl mx-auto mb-16 ${isRTL ? 'text-right' : ''}`}>
            <h2 className="text-display-sm text-foreground mb-4">
              {t(featuresSectionTitleKey)}
            </h2>
            <p className="text-lg text-muted-foreground">
              {t(featuresSectionDescKey)}
            </p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((feature) => (
              <div key={feature.titleKey} className={`bg-card rounded-2xl p-6 shadow-card hover:shadow-elevated transition-all duration-300 hover:-translate-y-1 ${isRTL ? 'text-right' : ''}`}>
      ...
      {/* Additional content (like calculators) */}
      {children}
